...
const getScoreReferenciasComerciales = async (id_certification, algoritmo_v, customUuid) => {
  ...
  try {
    ...
    if (calificaciones.length > 0) {
      for (const item of calificaciones) {
        if (typeof item.calificacion_referencia === 'string') {
          ...
          if (calificacionLowercase === 'mala') {
            ...
            countBuena++
          } else if (calificacionLowercase === 'regular') {
            countRegular++
          }
        }
      ...
      console.log('Buenas: ', countBuena)
      console.log('Regulares: ', countRegular)

      logger.info(`${fileMethod} | ${customUuid} Malas: ${countMala}}`)
      logger.info(`${fileMethod} | ${customUuid} Buenas: ${countBuena}}`)
      logger.info(`${fileMethod} | ${customUuid} Regulares: ${countRegular}}`)
...
const dataReporteCredito = async (id_certification, monto_solicitado, plazo) => {
  const fileMethod = `file: src/controllers/api/certification.js - method: dataReporteCredito`
  try {
    const now = new Date();
    ...
    if (!id_certification || !monto_solicitado || !plazo || !algoritmo_v) {
      ...
      return next(boom.badRequest(`La información que proporcionas esta incompleta`))
    }
    ...
    if (rotacion_ctas_x_cobrar.error) {
      ...
      reporteCredito._15_rotacion_ctas_x_cobrar = {
        ...
        limite_superior: algoritmo_v.v_alritmo == 2 ? 0 : rotacion_ctas_x_cobrar.limite_superior
      }
    }
